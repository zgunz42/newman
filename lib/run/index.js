...
var _ = require('lodash'),
    ...
    uploadAnalytics = require('./airtable-analytics'),
        ...
        runner.run(options.collection, {
            ...
            _.assignIn(callbacks, {
                 ...
                 */
                    ...
                    debugger;
                    if (err) {
                        return uploadAnalytics(err, emitter.summary, options)
                            .then(() => {
                    ...
                    asyncEach(emitter.exports, exportFile, function (err) {
                        ...
                        uploadRunToPostman(emitter.summary, options)
                            .then((runPayloadSize) => {
                                publishSummary = { runPayloadSize };

                                return uploadAnalytics(err, emitter.summary, options, publishSummary);
                            })
                            .then(() => { return callback(err, emitter.summary, publishSummary); })
                            .catch((err) => {
                                print.lf(err.message);

                                return callback(err, emitter.summary);
                            });

                        // @TODO: Uncomment when history server side changes are in
                        // if (options.publish) {
                        // fields present ->  resultUploaded , resultUrl , error , errorMessage , uploadPayloadSize
                        // let publishSummary;
                        ...
                        // }).finally(() => {
                        //     uploadAnalytics(err, emitter.summary, options, publishSummary)
                        //         .then(() => {
