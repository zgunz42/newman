 ...
 */
 _buildRequestObject = (request)=> {
    return {
    url: buildUrl(request.url), // TODO : Ask utkarsh if we have a function that combines a url object to
...
assertions && assertions.forEach(assert => {
    tests.push({
        ...
        error: assertion.error || {}, // TODO - Understand This is the entire object with params - name , index, test, message , stack
    ...
    executions.forEach( iter => {
        ...
        iter.forEach(req => {
            iteration.push({
                ...
                response: !skipResponse ? _buildResponseObject(req.response): null,
 ...
 */
 _buildPostmanUploadPayload = (runOptions, runSummary) => {
    const run = {
        name: runOptions.collection.nam || '',
 ...
 */
 uploadRunToPostman = (runSummary, runOptions) => {
    ...
    runOptions.collectionUID = util.extractResourceUID(options.collection);
    runOptions.environmentUID = util.extractResourceUID(options.environment);

     try{
         runPayload = _buildPostmanUploadPayload(runSummary, runOptions)
    }catch(error){
        throw new Error(`Unable to serialize the run - ${error}`);
    }

    runPayloadSize = sizeof(runPayload);
    ...
    return _uploadWithRetry(_upload(uploadOptions), retryOptions)
            .then(response => Promise.resolve({...response, runPayloadSize}));
}
